...
def dcp_knownprojects() -> pd.DataFrame:
    ...
    df = pd.read_csv(f"{current_dir}/raw/{filename}", dtype=str)
    return df

@ETL
def esd_projects() -> pd.DataFrame:
    filename = "2021.2.10 State Developments for Housing Pipeline.xlsx"
    df = pd.read_excel(f"{current_dir}/raw/{filename}", dtype=str)
    return df
...
def dcp_n_study_projected() -> gpd.geodataframe.GeoDataFrame:
    ...
    df = gpd.read_file(f"zip://{current_dir}/raw/{filename}")
    return df
...
def hpd_pc() -> pd.DataFrame:
    filename = "2021_4_8 DCP_SCA Pipeline.xlsx"
    df = pd.read_excel(f"{current_dir}/raw/{filename}", dtype=str)
    return df
...
def edc_dcp_inputs() -> gpd.geodataframe.GeoDataFrame:
    ...
    df = gpd.read_file(f"zip://{current_dir}/raw/{filename}.zip!{filename}/{filename}.shp")
    return df
