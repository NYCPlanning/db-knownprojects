...
dff["id"] = dff.apply(
    lambda x: [
        x["a_source"] + x["a_record_id"] + x["a_record_name"],
        x["b_source"] + x["b_record_id"] + x["b_record_name"],
    ],
    axis=1,
)
...
def dedup_exacts(group):
    ...
    if group.shape[0] > 1:
        if group[group["units_gross"] == 99999].shape[0] == 0:
            ...
            print(
                "All records in this cluster have missing unit data, skipping cluster..."
            )
    return group


dfff.sort_values(by=["cluster_id", "source_id"])
dfff.units_gross.fillna(
    value=99999, inplace=True
)  # Temporarily fill null so that it can be used as groupby
deduped = dfff.groupby(["cluster_id", "units_gross"], as_index=False).apply(
    dedup_exacts
)
...
to_carto(gdf, f"clusters_{year}", if_exists="replace", geom_col="geom")

# Export only unresolved clusters for review
print("Exporting unresolved cluster table for review...")
unresolved = deduped_export[~deduped_export["cluster_id"].isin(remove_clusters)].drop(
    columns=["units_net"]
)
...
gdf_unresolved = gpd.GeoDataFrame(unresolved)
gdf_unresolved["geometry"] = gdf_unresolved.geom.apply(lambda x: wkb.loads(x, hex=True))
to_carto(
    gdf_unresolved, f"clusters_unresolved_{year}", if_exists="replace", geom_col="geom"
)
