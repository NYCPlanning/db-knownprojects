...
df.loc[df["project_id"].isna(), "project_id"] = pd.Series(
    range(largest_project_id, largest_project_id + num_nulls)
).astype(str)
df.loc[~df["project_id"].str.contains("-", na=False), "project_id"] = (
    df["project_id"] + "-1"
)
