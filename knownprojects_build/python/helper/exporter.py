...
def exporter(
    df, output_table, DDL, con=edm_engine, sql="", sep=",", geo_column="", SRID=4326
):
    # parse output table
    ...
    if geo_column != "":
        df.loc[:, geo_column] = df.loc[:, geo_column].apply(
            lambda x: f"SRID={SRID};{x}" if x else ""
        )
    else:
        pass
    # export
...
def exporter_classic(df, output_table, DDL, con=edm_engine, sql="", chunksize=10000):
    ...
    for key, value in DDL.items():
        con.connect().execute(
            f'ALTER TABLE {output_table} ALTER COLUMN "{key}" TYPE {value} USING ("{key}"::{value});'
        )
