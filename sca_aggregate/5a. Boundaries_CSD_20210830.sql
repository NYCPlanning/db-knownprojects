...
(
	SELECT
		...
		a.senior_housing,
		a.inactive,
		ST_MakeValid(b.the_geom) as CSD_geom,
		b.SCHOOLDIST AS CSD,
	...
	on 
		...
		case
			when (st_area(st_makevalid(a.the_geom)::geography)>10000 or units_gross > 500) and a.source in('DCP Application','DCP Planner-Added PROJECTs') 		then
				st_dwithin(st_makevalid(a.the_geom)::geography,CSD_geom::geography,500)
			when st_area(st_makevalid(a.the_geom)) > 0 																											then
				st_dwithin(st_centroid(st_makevalid(a.the_geom))::geography,CSD_geom::geography,500)
			else
				st_dwithin(st_makevalid(a.the_geom)::geography,CSD_geom::geography,500)																			end
)
